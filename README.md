...
这个仓库是 `claude-for-legal-ZH` 的 Codex 包装层，不直接替代上游仓库。

安装前需要先准备上游仓库到本机默认位置：

- `~/.codex/vendor/claude-for-legal-ZH`

如果你的上游仓库不在这个位置，安装时可通过环境变量指定：

- `CODEX_FOR_LEGAL_UPSTREAM=/your/path/to/claude-for-legal-ZH`
...
不要再外包一层 `codex-for-legal-CN/` 目录，否则克隆后的结构会错误。
