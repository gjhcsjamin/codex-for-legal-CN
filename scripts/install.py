...
UPSTREAM = Path(
    os.environ.get("CODEX_FOR_LEGAL_UPSTREAM", str(CODEX_HOME / "vendor" / "claude-for-legal-ZH"))
).expanduser()
CODEX_SKILLS = CODEX_HOME / "skills"
LOCAL_VENDOR = CODEX_HOME / "vendor" / "codex-for-legal-CN"
...
- 上游模块：`~/.codex/vendor/claude-for-legal-ZH/{name}`
- 说明文档：`~/.codex/vendor/claude-for-legal-ZH/{name}/README.md`
- 主提示：`~/.codex/vendor/claude-for-legal-ZH/{name}/CLAUDE.md`
...
def main() -> None:
    ...
    if not UPSTREAM.exists():
        raise SystemExit(
            ...
            f"{UPSTREAM}\n"
            "Install or copy CSlawyer1985/claude-for-legal-ZH to that path first, "
            "or set CODEX_FOR_LEGAL_UPSTREAM to your local upstream path."
        )
    if LOCAL_VENDOR.exists():
        raise SystemExit(f"destination exists: {LOCAL_VENDOR}")

    shutil.copytree(REPO_ROOT, LOCAL_VENDOR)

    root_skill = (REPO_ROOT / "templates/root-skill.md.tpl").read_text(encoding="utf-8")
